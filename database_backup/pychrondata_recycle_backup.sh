...
if  $BACKUP_COMMAND | gzip > $BACKUP_DIR/pychrondata_recycle.gz 2>>$BACKUP_LOG_DIR/$BACKUP_LOG_FILE ; then
	echo "`date` Backup Successful to file $BACKUP_DIR/pychrondata_recycle.gz" >> $BACKUP_LOG_DIR/$BACKUP_LOG_FILE
else
